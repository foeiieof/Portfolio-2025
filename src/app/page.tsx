...
import { motion, useScroll, useMotionValue, useVelocity, useSpring, useTransform, useAnimationFrame } from "motion/react"
import { useRouter } from "next/navigation"
import { elementStyle, MotionDiv } from "./components/Motiondiv"
...
const ContinuousSlider = ({ drt, setDrt }: { drt: number, setDrt: Dispatch<SetStateAction<number>> }) => {
  ...
  return (
    <div className="overflow-hidden w-full h-full sm:h-[600px] relative">
      <motion.div
        onMouseOver={() => { setDrt(2) }}
        onMouseLeave={() => { setDrt(4) }}
        style={{ x }}
...
const Home = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const clampedValue = Math.max(0, Math.min(scrollY / maxScroll, 1));
